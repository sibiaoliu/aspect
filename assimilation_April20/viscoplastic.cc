  ...
  namespace MaterialModel
  {
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      for (unsigned int i=0; i < in.temperature.size(); ++i)
        {
    	  //Loading the age data file
    	  const InitialTemperature::Assimilation <dim> &age_assim
    	        = Plugins::get_plugin_as_type<const InitialTemperature::Assimilation <dim> >
    	                               (this->get_initial_temperature());
    	  const double real_age = age_assim.ascii_age(in.position[i]);
    ...
    template <int dim>
    void
    ViscoPlasticAssim<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic Assim");
        {
    ...
    template <int dim>
    void
    ViscoPlasticAssim<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic Assim");
        {
          ...
          // Rheological parameters
          pre_yield_viscosity_factors = Utilities::possibly_extend_from_1_to_N (Utilities::string_to_double(
        		  Utilities::split_string_list(prm.get("Pre-yield viscosity multiplicative factors"))),
                  n_fields, "Pre-yield viscosity multiplicative factors");
    ...
    template <int dim>
    void
    ViscoPlasticAssim<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlasticAssim,
                                   "visco_plastic_assim",
