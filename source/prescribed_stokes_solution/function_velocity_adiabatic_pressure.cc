...
#include <aspect/prescribed_stokes_solution/function_velocity_adiabatic_pressure.h>
...
namespace aspect
{
  ...
  {
    template <int dim>
    FunctionVelocityAdiabaticPressure<dim>::FunctionVelocityAdiabaticPressure ()
      :
    ...
    template <int dim>
    void
    FunctionVelocityAdiabaticPressure<dim>::stokes_solution (const Point<dim> &position, Vector<double> &value) const
    {
      // velocity
      ...
      if (this->get_parameters().include_melt_transport)
        {
          // fluid pressure
          value(dim) = this->get_adiabatic_conditions().pressure(position);
          ...
          // pressure
          value(2*dim+2) = this->get_adiabatic_conditions().pressure(position);
        }
      else
        {
          // pressure
    ...
    template <int dim>
    void
    FunctionVelocityAdiabaticPressure<dim>::update()
    {
    ...
    template <int dim>
    void
    FunctionVelocityAdiabaticPressure<dim>::declare_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    FunctionVelocityAdiabaticPressure<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Prescribed Stokes solution");
      {
        prm.enter_subsection("Velocity function");
...
namespace aspect
{
  namespace PrescribedStokesSolution
  {
    ASPECT_REGISTER_PRESCRIBED_STOKES_SOLUTION(FunctionVelocityAdiabaticPressure,
                                               "function velocity adiabatic pressure",
                                               "This plugin allows to prescribe the Stokes solution "
                                               "for the velocity field in terms of an "
                                               "explicit formula. The format of these "
                                               "functions follows the syntax understood by the "
                                               "muparser library, see Section~\\ref{sec:muparser-format}. "
                                               "The pressure is prescribed to follow to initially computed "
                                               "adiabatic profile as stored in the adiabatic conditions plugin.")
  }
}
