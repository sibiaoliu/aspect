...
#include <aspect/postprocess/visualization/surface_maximum_horizontal_compressive_stress.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      template <int dim>
      void
      SurfaceMaximumHorizontalCompressiveStress<dim>::
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          {
            ...
            Tensor<1,dim> surface_maximum_horizontal_compressive_stress;
            switch (dim)
              {
                ...
                case 3:
                {
                  ...
                  const double surface_maximum_horizontal_compressive_stress_magnitude
                  ...
                  const double surface_minimum_horizontal_compressive_stress_magnitude
                    = (n_perp * ((compressive_stress
                                  -
                                  in.pressure[q] * unit_symmetric_tensor<dim>()) * n_perp));

                  surface_maximum_horizontal_compressive_stress
                    = n * (surface_maximum_horizontal_compressive_stress_magnitude -
                           surface_minimum_horizontal_compressive_stress_magnitude);
            ...
            for (unsigned int i=0; i<dim; ++i)
              computed_quantities[q](i) = surface_maximum_horizontal_compressive_stress[i];
          }
      }
      ...
      SurfaceMaximumHorizontalCompressiveStress<dim>::get_names () const
      {
        return std::vector<std::string> (dim, "surface_maximum_horizontal_compressive_stress");
      }


      template <int dim>
      std::vector<DataComponentInterpretation::DataComponentInterpretation>
      SurfaceMaximumHorizontalCompressiveStress<dim>::get_data_component_interpretation () const
      {
        return
          std::vector<DataComponentInterpretation::DataComponentInterpretation>
          (dim,
      ...
      template <int dim>
      UpdateFlags
      SurfaceMaximumHorizontalCompressiveStress<dim>::get_needed_update_flags () const
      {
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(SurfaceMaximumHorizontalCompressiveStress,
                                                  "surface maximum horizontal compressive stress",
                                                  "A plugin that computes the direction and magnitude of the "
                                                  "surface maximum horizontal component of the compressive stress as a vector "
